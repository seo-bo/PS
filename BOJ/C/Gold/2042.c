...
long long init(long long * ary, long long start, long long end, long long node)
{
	...
	return tree[node] = init(ary, start, mid, node * 2) + init(ary, mid + 1, end, node * 2 + 1);
}
...
void update(long long start, long long end, long long node, long long idx, long long dif)
{
	...
	for (int i = 0; i < test; i++)
	{
		...
		switch (temp)
		{
			case 1:
			{
				...
				getchar();
				long long diff = temp2 - ary[temp1 - 1];
				ary[temp1 - 1] = temp2;
				update(0, num - 1, 1, temp1 - 1, diff);
				break;
			}
			case 2:
			{
